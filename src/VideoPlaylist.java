...
VideoPlaylist(Stage st,Boolean finit,int n){
    init = finit;
    next = n;
    fp = new BorderPane();
    lp = new BorderPane();
...
mainScreen.setOnMouseEntered(e->{
    player.play();
});
...
me = new EventHandler<MouseEvent>() {
    ...
    public void handle(MouseEvent event) {
       ...
       if(button==MouseButton.SECONDARY){
        ...
        if(App.queue.contains(links.get(""+toremove))){
...
remove.setOnAction(ae->{
    ...
    try{
    FileReader fr=new FileReader("linkk.txt"); 
    ...
    FileReader fr2=new FileReader("main.txt"); 
...
public static void setbutton(int i,String ile,LinkedList<Node> videolList){
  ...
  Media m= new Media(ile);
  MediaPlayer player = new MediaPlayer(m);
  MediaView mediaView = new MediaView(player);
  mediaView.setFitHeight(170);
  mediaView.setFitWidth(170);
  bb[i].setPrefSize(185,110);
  bb[i].setGraphic(mediaView);
...
if(next == 0){
    ...
    p.getStylesheets().add("Styles/Script.css");      
    fp.setCenter(p);
    settPane(fp);
}
else if(next == 1){
...
public static void storelinks(Button key,String valu){
    try{
        FileWriter ffw  = new FileWriter("main.txt",true);
        bw = new BufferedWriter(ffw);

        FileWriter ffw2  = new FileWriter("linkk.txt",true);
        bw2 = new BufferedWriter(ffw2);
